...
def train_net(args, ctx, pretrained_res, pretrained_vgg, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
	...
	# arg_params, aux_params = load_param(pretrained_res, epoch, convert=True)
        ...
        init_fcnxs.init_from_fcnxs(ctx[0], sym, arg_params, aux_params)
        # init_fcnxs.init_from_fcnxs(ctx[0], sym, arg_params_vgg, aux_params_vgg)
...
def main():
    ...
    train_net(args, ctx, config.network.pretrained_res, config.network.pretrained_vgg, config.network.pretrained_epoch,
