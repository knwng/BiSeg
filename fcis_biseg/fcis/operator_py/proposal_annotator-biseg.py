...
class ProposalAnnotatorOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        rois, labels, bbox_targets, bbox_weights, mask_reg_targets, mask_ss_targets = \
            self.sample_rois(all_rois, fg_rois_per_image, rois_per_image, self._num_classes,
                             self._cfg, gt_boxes=gt_boxes, gt_masks=gt_masks, ss_masks=ss_masks)

        for ind, val in enumerate([rois, labels, bbox_targets, bbox_weights, mask_reg_targets, mask_ss_targets]):
    ...
    def sample_rois(self, rois, fg_rois_per_image, rois_per_image, num_classes, cfg,
                    labels=None, overlaps=None, bbox_targets=None, gt_boxes=None, gt_masks=None, ss_masks=ss_masks):
        if labels is None:
        ...
        mask_ss_targets = -np.ones((len(keep_indexes), 1, self._mask_size, self._mask_size))
        for idx, obj in enumerate(fg_indexes):
            ...
            # add semantic segmentation masks
            ss_mask = ss_masks[gt_assignment[obj]]
            mask_ss_target = intersect_box_mask(ex_roi, gt_roi, ss_mask)
            mask_ss_target = cv2.resize(mask_ss_target.astype(np.float), (self._mask_size, self._mask_size))
            mask_ss_target = mask_ss_target >= self._binary_thresh
            mask_ss_targets[idx, ...] = mask_ss_target

        return rois, labels, bbox_targets, bbox_weights, mask_reg_targets, mask_ss_targets
...
class ProposalAnnotatorProp(mx.operator.CustomOpProp):
    ...
    def list_arguments(self):
        return ['rois', 'gt_boxes', 'gt_masks', 'ss_masks']

    def list_outputs(self):
        return ['rois_output', 'label', 'bbox_target', 'bbox_weight', 'mask_reg_targets', 'mask_ss_targets']

    def infer_shape(self, in_shape):
        rpn_rois_shape = in_shape[0]
        gt_boxes_shape = in_shape[1]
        gt_masks_shape = in_shape[2]
        ss_masks_shape = in_shape[3]
        ...
        mask_ss_targets_shape = (rois, 1, self._mask_size, self._mask_size)

        return [rpn_rois_shape, gt_boxes_shape, gt_masks_shape, ss_masks_shape], \
               [output_rois_shape, label_shape, bbox_target_shape, bbox_weight_shape, mask_reg_targets_shape, mask_ss_targets_shape]
